...
import {
  userFeedItems, allUserRSSSources, markItemRead, saveItem, markUserFeedItemsRead,
  addUserSource, addUserPodcast, removeUserSource, getItemsByCategory,
...
useEffect(() => {
  ...
  const handleAddSource = async () => {
    ...
    try {
      ...
      toast.success(`${feedType === "rss" ? "Feed" : "Podcast"} added successfully!`);
      setNewSourceUrl("");
  ...
  <>
    ...
    <main className="flex-1 p-4 max-w-full overflow-x-hidden">
        ...
        <div className="flex mb-6 w-full max-w-full">
          ...
          >
            {addingSource ? "Adding..." : `Add ${feedType === "rss" ? "Feed" : "Podcast"}`}
          </Button>
        </div>
        ...
        <section className="mt-10 w-full max-w-full">
          ...
          {loading ? (
            ...
            <div className="w-full text-center py-10 text-gray-400">No items yet.</div>
          ) : (
            <div className="flex flex-col divide-y divide-gray-300">
