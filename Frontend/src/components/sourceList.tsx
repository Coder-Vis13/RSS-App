...
export default function FullSource() {
  ...
  return (
    <div>
      ...
      {sourceTitle && <h2 className='py-5 border-b'><b>Source: </b>{sourceTitle}</h2>}
      <ul>
        {sourceArticles.map((item, i) => (
            <li key={i} className="mb-4 border-b pb-2 pt-1">
                <div><b>Title:</b> <a href={item.link} target="_blank" rel="noreferrer">{item.title}</a></div>
                {item.creator && <div><b>Author:</b> {item.creator}</div>}
                {item.description && <div><b>Description:</b> {item.description}</div>}
                {item.summary && <div><b>Summary:</b> {item.summary}</div>}
                {item.pubDate && <div><b>Published:</b> {new Date(item.pubDate).toLocaleDateString()}</div>}
            </li>
        ))}
      </ul>
    </div>
